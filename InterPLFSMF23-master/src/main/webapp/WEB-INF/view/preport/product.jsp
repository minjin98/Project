...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <h1 class="mt-4">${pnames.prodName}</h1>
                        </c:forEach>
                    </div>
                    <div class="container-fluid px-4 text-end">
                    	<form action = "${path}/preport/product?prodNo=KBD003&planID=KBPL02" method ="post">
                    		<span>계획코드</span>
                    		<select>
		                    	<c:forEach var="pnames" items="${p_names}">
		                        <option>${pnames.planID}</option>
		                        </c:forEach>
	                        </select>
	                        ...
	                        <div class="col-xl-3 col-md-6">
	                            <div class="card bg-primary text-white mb-4">
	                                <div class="card-body">제품명 : 
	                                	<c:forEach var="pnames" items="${p_names}">
		                            	<a>${pnames.prodName}</a> <!-- 한 건만 가져오는 것이기 때문에 forEach를 사용할 필요가 없음 -->
		                            	</c:forEach>
			                        </div>
	                            </div>
	                        </div>
	                        ...
	                        <div class="col-xl-3 col-md-6">
	                            <div class="card bg-success text-white mb-4">
	                                <div class="card-body">시작일 : 
	                                	<c:forEach var="ppr" items="${p_pr}">
		                            	<a>${ppr.startdate}</a> <!-- 한 건만 가져오는 것이기 때문에 forEach를 사용할 필요가 없음 -->
		                            	</c:forEach>
			                        </div>
	                            </div>
	                        </div>
                            ...
                            <div class="col-lg-5">
                                <div class="card mb-4">
                                    ...
                                    <div class="card-footer small text-muted">
                                    	<c:forEach var="ppr" items="${p_pr}">
                                    	양품 : ${ppr.passedQty} &nbsp;&nbsp;&nbsp; 불량품 : ${ppr.failedQty}
                                    	</c:forEach>
                                    </div>
                                </div>
                            </div>
	                                ...
	                                <div class="card-body">
		                                <div class="table-responsive">
						                	<table class="table text-nowrap mb-0 align-middle">
							                    ...
							                    <tbody>
							                    	<c:forEach var="pissue" items="${p_issue}">
								                      	<tr>
								                        	<td class="border-bottom-0"><h6 class="fw-semibold mb-0">${pissue.arm_Seq}</h6></td>
								                        	<td class="border-bottom-0">
								                            	<h6 class="fw-semibold mb-1">${pissue.issueNo}</h6>
								                        	</td>
									                        <td class="border-bottom-0">
									                        	<div class="d-flex align-items-center gap-2">
								                            	<span class="badge bg-primary rounded-3 fw-semibold">${pissue.issueName}</span>
								                          		</div>
									                        </td>
								                       		<td class="border-bottom-0">
								                          		<p class="mb-0 fw-normal">${pissue.issueInfo}</p>
								                        	</td>
								                        	<td class="border-bottom-0">
								                          		<h6 class="fw-semibold mb-0">${pissue.timestamp}</h6>
								                        	</td>
								                      	</tr>
							                      	</c:forEach>
						                      	</tbody>
                        ...
                        <div class="card mb-4" >
                            ...
                            <div class="card-body" >
                                <table id="datatablesSimple">
                                    ...
                                    <tbody>
	                                <c:forEach var="pinven" items="${p_inven}">
                                        <tr>
                                            <td>${pinven.materNo}</td>
                                            <td>${pinven.materName}</td>
                                            <td>${pinven.materPrice}</td>
                                            <td>${pinven.materQty}</td>
                                        </tr>
    	                            </c:forEach>
                                    </tbody>
                                </table>
        ...
        <script>
        	<c:forEach var="ppr" items="${p_pr}">
           	drawPieChart([${ppr.passedQty}, ${ppr.failedQty}]);
           	</c:forEach>
        </script>
    </body>
</html>
